...
if(isset($_GET) and !empty($_GET)){
	if((isset($_GET['libro']) && !empty($_GET['libro'])) && (isset($_GET['cedula']) && !empty($_GET['cedula']))){
		...
		if(count($resultados) > 0){
			...
			$strQuery = sprintf("SELECT p.id FROM personas p JOIN suspendidos s p.id=s.persona_id WHERE p.cedula='%s' AND s.hasta > CURDATE()", 
				$_GET['cedula']);		
			$conexion = new Conexion($database);
			$resultados = $conexion->seleccionarDatos($strQuery);	
			if(count($resultados) > 0){			
			...
			if($resultados and !empty($resultados)){
				...
				$strQuery = sprintf("SELECT * FROM libros WHERE id=%d", $_GET['libro']);
				$resultados = $conexion->seleccionarDatos($strQuery);
				$libro = $resultados[0];
