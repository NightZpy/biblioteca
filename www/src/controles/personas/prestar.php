...
if(Sesion::existe('usuario')){
	...
	if(isset($_GET) and !empty($_GET)){
		if((isset($_GET['libro_id']) and !empty($_GET['libro_id'])) and (isset($_GET['cedula']) and !empty($_GET['cedula'])) and (isset($_GET['cota']) and !empty($_GET['cota']))){
			...
			$strQuery = sprintf("SELECT pr.id FROM prestamos pr JOIN personas p ON pr.persona_id=p.id WHERE pr.fecha_entregado IS NULL AND p.cedula='%s' AND pr.cota_id=%d",$_GET['cedula'], $_GET['cota']);
					...
					if(count($resultados) > 0){	
						...
						if($conexion->actualizarDatos($strQuery)){
							...
							if($conexion->agregarRegistro($strQuery)){
								...
								$strQuery = sprintf("SELECT p.*, u.nombres AS nombre_usuario, u.apellidos AS apellido_usuario, c.nombre AS cota FROM prestamos p JOIN cotas c ON p.cota_id=c.id JOIN usuarios u ON p.usuario_id=u.id WHERE p.id=%d", $prestamo);
								$resultados = $conexion->seleccionarDatos($strQuery);				
