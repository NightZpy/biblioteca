...
if(Sesion::existe('usuario')){
	...
	if(isset($_GET) and !empty($_GET)){
		if((isset($_GET['libro_id']) and !empty($_GET['libro_id'])) and (isset($_GET['cedula']) and !empty($_GET['cedula'])) and (isset($_GET['ejemplar']) and !empty($_GET['ejemplar']))){
			...
			//verifico si la persona ya tiene prestamos
			$strQuery = sprintf("SELECT pr.id FROM prestamos pr JOIN personas p ON pr.persona_id=p.id WHERE pr.fecha_entregado IS NULL AND p.cedula='%s'",$_GET['cedula']);
			...
			if(!(count($resultados) > 0)){
				...
				Sesion::setValor('error', $warnings['SUSPENSION']);
				header('Location: '.CONTROL_HTML.'/libros/prestar.php?libro_id='.$_GET['libro_id']);				
			}
		} else {
			$error = true;		
