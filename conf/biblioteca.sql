...
-- Tiempo de generación: 08-05-2013 a las 03:17:52
...
CREATE TABLE IF NOT EXISTS `carreras` (
  ...
  `nombre` varchar(60) NOT NULL COMMENT 'Nombre de la carrera',
  PRIMARY KEY (`id`)
...
CREATE TABLE IF NOT EXISTS `estudiantes` (
  ...
  `carrera_id` int(10) unsigned NOT NULL COMMENT 'Cógido asociado perteneciente a la carrera que cursa',
  PRIMARY KEY (`id`),
...
CREATE TABLE IF NOT EXISTS `libros` (
  ...
  `idioma` varchar(80) NOT NULL DEFAULT 'español',
  `condicion` varchar(30) NOT NULL COMMENT 'Si se obtuvo nuevo o usado',
...
INSERT INTO `libros` (`id`, `codigo`, `autor`, `titulo`, `descripcion`, `editorial`, `ejemplar`, `idioma`, `condicion`, `fecha_ingreso`, `categoria_id`) VALUES
(2, 'abcdef', 'Laurent', 'Calculo ', 'Calculos con derivadas e integrales', 'Numeric Editorial', 1, 'español', 'nuevo', '2013-02-02', 1),
(3, 'axcdeg', 'José', 'Geometria ', 'Calculos y representación geometrica', 'Santillana', 1, 'español', 'nuevo', '2013-01-02', 1);
...
CREATE TABLE IF NOT EXISTS `prestamos` (
  `id` int(10) unsigned NOT NULL AUTO_INCREMENT,
  `estudiante_id` int(10) unsigned NOT NULL COMMENT 'Id asociado al estudiante que realizó el prestamo',
  ...
  PRIMARY KEY (`id`),
  UNIQUE KEY `usuario_id` (`estudiante_id`,`libro_id`),
...
CREATE TABLE IF NOT EXISTS `supendidos` (
  ...
  `estudiante_id` int(10) unsigned NOT NULL COMMENT 'Id asociado al estudiante que ha sido suspendido del servicio ',
  `hasta` date NOT NULL COMMENT 'Fecha hasta la que permanecerá suspendido el estudiante',
  PRIMARY KEY (`id`),
  KEY `libro_id` (`libro_id`,`estudiante_id`),
  KEY `usuario_id` (`estudiante_id`)
) ENGINE=InnoDB DEFAULT CHARSET=latin1 AUTO_INCREMENT=1 ;
...
ALTER TABLE `estudiantes`
  ADD CONSTRAINT `estudiantes_ibfk_1` FOREIGN KEY (`carrera_id`) REFERENCES `carreras` (`id`) ON DELETE NO ACTION ON UPDATE CASCADE;
...
ALTER TABLE `prestamos`
  ADD CONSTRAINT `prestamos_ibfk_3` FOREIGN KEY (`estudiante_id`) REFERENCES `estudiantes` (`id`) ON DELETE NO ACTION ON UPDATE CASCADE,
...
ALTER TABLE `supendidos`
  ADD CONSTRAINT `supendidos_ibfk_2` FOREIGN KEY (`estudiante_id`) REFERENCES `estudiantes` (`id`) ON DELETE NO ACTION ON UPDATE CASCADE,
